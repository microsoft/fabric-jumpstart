...
1. Create a new YAML file in `src/fabric_jumpstart/fabric_jumpstart/jumpstarts/community/` (or `core/` for Microsoft-sponsored jumpstarts):
   ...
   - Required fields (_start by copying and editing an existing YAML file_):
     ...
     - `install_options` _(optional)_: List of kebab-case install variants (e.g., industries). Each option must have its own subfolder under `workspace_path` containing that variant's Fabric items (e.g., `my-jumpstart/healthcare/`). Users select variant(s) at install time; the website renders a dropdown for the options and updates the install snippet accordingly. Two forms:
       - Single (string): `jumpstart.install('<logical-id>', install_option='healthcare')` — deploys only that option's folder.
       - Multi (list): `jumpstart.install('<logical-id>', install_option=['healthcare', 'retail-sales'])` — deploys each option into the same workspace, one workspace folder per option, with option-prefixed item names (e.g., `retail_sales_my_lakehouse`) so options never collide. Later installs can add more options to the same workspace the same way; a single-option list (`['healthcare']`) uses the same folder+prefix layout so additions always compose.
     ...
     - `data_load` _(optional)_: Declarative sample-data load executed by the installer after deployment — entirely via REST APIs, with no notebook execution and no code from the content repository ever running. Use this instead of a "run this notebook first" step so the install is a single command. Fields:
       - `source`: Repo-relative path to a folder or `.zip` archive of CSV and/or Parquet files. Supports the `{install_option}` token (e.g., `my-jumpstart/data/{install_option}_package.zip`). Parquet is preferred: when a table exists in both formats, the Parquet file wins (real column types, smaller payloads; handled via `pyarrow`, preinstalled in Fabric notebooks).
       - `shift_timestamps_to_now` _(optional)_: When `true`, all timestamp-like columns (CSV and Parquet, typed or string) are shifted by a single global day-offset so the newest event lands ~yesterday (keeps demo data "fresh"; idempotent).
       - `lakehouse_tables` _(optional)_: `{lakehouse: <name>, archive_path: <prefix>}` — each CSV/Parquet under the prefix becomes a Delta table (Load Table API with per-format options; falls back to a short-lived Livy session for schema-enabled lakehouses).
       - `kusto_tables` _(optional)_: `{database: <name>, archive_path: <prefix>}` — each CSV/Parquet under the prefix becomes a KQL table (schema from Parquet types when available, inferred from samples for CSV; inline ingestion).
