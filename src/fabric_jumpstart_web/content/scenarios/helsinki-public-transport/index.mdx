...
The jumpstart deploys the Fabric back end. The matching front end — a live map that renders the fleet on a 2D Leaflet map **and** on a 3D city twin — ships separately as a template in the [Awesome Rayfin](https://github.com/microsoft/awesome-rayfin) gallery:
...
The 3D mode renders vehicles on the City of Helsinki's own semantic city model — **no Cesium ion token, no Google Maps API key**. Every layer is CC BY 4.0 open data streamed straight from `kartta.hel.fi`, which sends CORS headers, so nothing has to be re-hosted:
...
![The 3D city view in the Fabric portal](images/02-3d-city.webp)

The city also publishes photogrammetric reality meshes, and they look better from the air — but vehicles are clamped to the *terrain*, and the mesh surface sits several metres above it, so zoomed in the buses sank under the buildings and vanished. A transit map that hides the transit is not a trade-off, so the app ships one surface:

![Street-level detail on the LoD2 buildings](images/03-street-level.webp)
